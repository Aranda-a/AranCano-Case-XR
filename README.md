...
**我的角色：** 本项目为**独立完成的 0–1 原型**——问题定义、主路径交互、多通道反馈、真机验证与埋点口径均由作者负责。开发与文档环节使用 AI 辅助提速；**产品命题、主路径取舍、体验标准与验证口径由我定义并验收。** 公开仓刻意不放核心实现，只展示决策与验证；约束来自脱敏试用反馈与预先口径，避免自嗨。
...
独立交互设计与产品原型 · 真机验证中（可演示）· **独立开发**。  
对外角色侧重：**产品原型 / 交互 / XR**；Annno 为服务主路径的陪伴型 IP，不单独作为「纯角色设定集」展示。入职后可按岗位深耕其一，与工程分工协作。
