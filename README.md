...
- **轻量数据卡与汇总口径**：[METRICS.md](docs/METRICS.md)
...
| 摩擦 | 现在怎么接 | 验证分层 |
|------|------------|----------|
| 比抓挠慢 | 微信直达；默认立体跟包（C），不行再走轻量平面（A） | 当场：轻量数据卡看完成率；打开到首次按下的时长可继续记 |
| 要对准 | 对准 = 把手从皮肤挪开的第一步 | 引导与出模关系讲清；试用口述已反馈到文案与主路径选择 |
| 下次还会不会开 | 封印、月历、批语做完成记忆；不靠排行榜或连续登录硬推 | **召回层**（机制）已预留；**需求层**（无人提醒时会不会打开）按 [验证计划](docs/VALIDATION.md) 日记法另轨验证 |
...
这些是定性线索，用来驱动取舍；定量观察见上方轻量数据卡。

---

## 5. 主路径之外（工程边界，简记）

- 设备空间能力不足时，**兼容退路**仍可用轻量平面走完按压—封印  
- 离开页面会停掉相机和异步任务；图像默认本地处理  
- 性能对外以统一机型基线为准；本页先交付交互与验证结论  
