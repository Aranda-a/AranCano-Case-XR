...
**角色标签：** 交互设计 · 产品原型 · XR 空间交互 · 陪伴型 IP（Annno）
...
- **有效样本**指按**预先口径入选**，不是事后只挑好看的局：完成率看「有小结的体验版会话」；想挠变化另要求前后都认真拖过（探针未齐的局只进完成率、不进差值）。  
- **有说服力的是方法，不是效应量**：能说明埋点 → 入选规则 → 小样本观察；**不能**证明止痒、显著或可推广。n&lt;30 只作可用性线索。  
- 数据来自 2026-09-13 飞书匿名本局小结导出；含开发自测与体验版，对外表用 trial。不作疗效结论。汇总步骤见 [METRICS.md](docs/METRICS.md)。
...
独立交互设计与产品原型 · 真机验证中。  
对外角色侧重：**交互 / 产品 / XR 原型**；Annno 为服务主路径的陪伴型 IP，不单独作为「纯角色设定集」展示。
